...
if not os.path.isdir(bioiddir):
  ...
  with zipfile.ZipFile(bioidfile, 'r') as zip:
    zip.extractall(bioiddir)
...
if not os.path.isdir(bioidptsdir):
  ...
  with zipfile.ZipFile(bioidptsfile, 'r') as zip:
    zip.extractall(bioidptsdir)
...
def runEyeCenter(id, mode):
  ...
  return [[int(y) for y in x.split('\t')] for x in result]


def calcError(id, predictions):
  result = []
  for pred in predictions:
    min_index, min_value = min(enumerate([math.sqrt((pred[0] - real[0])**2 + (pred[1] - real[1])**2) for real in testCases[id]]), key=operator.itemgetter(1))
    result.append(min_value)
  return result

for id in testCases:
  cutout = preprocessImage(id)
  try:
    predictions = runEyeCenter(id, 'naive')
    predictions = [[cutout[1][0] + x[0], cutout[1][1] + x[1]] for x in predictions]
    print(calcError(id, predictions))
    drawResultImage(id, predictions)
    # predictions = runEyeCenter(id, 'ascend')
    # predictions = runEyeCenter(id, 'evol')
  finally:
    os.remove(cutout[0])
